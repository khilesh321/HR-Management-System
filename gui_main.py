...
def view_leave_requests_gui():
    ...
    win.geometry("900x500")
    win.attributes("-topmost", True)
    from data.storage import get_leaves, update_leave_status
    ...
    for i, req in enumerate(leave_requests, start=1):
        ...
        for j, value in enumerate(values):
            ctk.CTkLabel(table, text=req.get(value, ''), width=20).grid(row=i, column=j, padx=2, pady=2)
        ...
        if req.get('status') == 'Pending':
            ...
            ctk.CTkButton(table, text="Approve", command=approve, fg_color="green").grid(row=i, column=len(columns), padx=2, pady=2)
            ctk.CTkButton(table, text="Reject", command=reject, fg_color="red").grid(row=i, column=len(columns)+1, padx=2, pady=2)
    if not leave_requests:
        ctk.CTkLabel(table, text="No leave requests found.", font=("Arial", 14)).grid(row=1, column=0, columnspan=len(columns), pady=20)
